...
class ChooseFriendViewController: UITableViewController {
    ...
    var delegate: FriendSelectDelegate!
    var datas = [AtData]()
    ...
    func loadAtList() {
        ...
        HttpUtil.GET(url: Urls.AtListUrl, params: nil) { (ok, res) in
            ...
            if self.datas.count == 0 { self.placeholderText = "未能获取到好友列表" }
    ...
    @IBAction func doneClick(_ sender: Any) {
        ...
        self.datas.forEach { (d) in
            if d.checked { names.append(d.nickname) }
        }
        delegate.selectFriends(names: names)
        presentingViewController?.dismiss(animated: true)
    }
