...
class HtmlAttr {
    ...
    static func parserAttr(type: HtmlTag, src: String) -> HtmlAttr? {
        ...
        case .A:
            ...
            attr.href = getAttr(src: src, start: start?.upperBound)?
                    .replacingOccurrences(of: "&amp;", with: "&", options: .literal)
        case .IMG:
            let start = src.range(of: "src")
            attr.src = getAttr(src: src, start: start?.upperBound)?
                    .replacingOccurrences(of: "&amp;", with: "&", options: .literal)
        case .FONT:
            attr.color = getTextColor(src: src)
            attr.bgColor = getBgColor(src: src)
        case .DIV : // div align="left"
            attr.textAlign = getAlign(src: src)
        default:
            break
        }
    ...
    //a="b" src="" href=""
    static func getAttr(src: String, start: String.Index?) -> String? {
        if let start = start {
    ...
    //attr css
    //color="#ff0000"
    static func getTextColor(src: String) -> UIColor? {
        if let s = src.range(of: "color") {
            if s.lowerBound > src.startIndex && (src[src.index(before: s.lowerBound)] == "-" ||
                    src[src.index(before: s.lowerBound)] == "g") {
    ...
    //left right center
    //或者文字布局 align="center"
    static func getAlign(src: String) -> NSTextAlignment? {
        if let s = src.range(of: "align") {
            if let f = getAttr(src: src, start: s.upperBound) {
                switch f {
                case "right":
                    return .right
                case "center":
